...
class RGBDFrame():
  ...
  def load(self, file_handle):
    ...
    tmp = struct.unpack('c'*self.color_size_bytes, file_handle.read(self.color_size_bytes))
    self.color_data = b''.join(tmp)

    tmp = struct.unpack('c'*self.depth_size_bytes, file_handle.read(self.depth_size_bytes))
    #tmp = [str(a) for a in tmp]
    self.depth_data = b''.join(tmp)
...
class SensorData:
  ...
  def load(self, filename):
    with open(filename, 'rb') as f:
      ...
      self.sensor_name = b''.join(tmp).decode()
